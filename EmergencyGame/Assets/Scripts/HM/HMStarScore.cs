using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class HMStarScore : MonoBehaviour
{
    Animator animator;

    [Header("Animation Names")]
    public string Star0 = "StarImg0";
    public string Star1 = "StarImg1";
    public string Star2 = "StarImg2";
    public string Star3 = "StarImg3";
    public string Star4 = "StarImg4";

    //string nowAni = "", oldAni = "";

    public int sceneIndex = 1; // �� �ε��� ����
    HMTimerController hmTimerController;

    void Start()
    {
        hmTimerController = FindObjectOfType<HMTimerController>();
        animator = GetComponent<Animator>();

        if (hmTimerController == null)
            Debug.LogWarning("������ TimerController�� ã�� �� �����ϴ�!");
    }

    void Update()
    {
        UpdateStarScore();
    }

    void UpdateStarScore()
    {
        if (hmTimerController == null) return;

        int starCount = 0;

        float timer = hmTimerController.totalTimer;


        if (timer <= 180 && timer > 144)
        {
            animator.Play(Star4);
            starCount = 4;
            //Debug.Log("��4");
        }
        else if (timer <= 144 && timer > 108)
        {
            animator.Play(Star3);
            starCount = 3;
            //Debug.Log("��3");
        }
        else if (timer <= 108 && timer > 72)
        {
            animator.Play(Star2);
            starCount = 2;
            //Debug.Log("��2");
        }
        else if (timer <= 72 && timer > 36)
        {
            animator.Play(Star1);
            starCount = 1;
            //Debug.Log("��1");
        }
        else
        {
            animator.Play(Star0);
            starCount = 0;
            //Debug.Log("��0");
        }

        //ChangeAnimation();

        //  GameDataManager�� ����
        if (GameDataManager.Instance != null)
            GameDataManager.Instance.SetStar(sceneIndex, starCount);
            //Debug.Log("starCount :" + starCount);

        if (GameDataManager.Instance != null)
        {
            GameDataManager.Instance.SetStar(sceneIndex, starCount);
            StartCoroutine(GameDataManager.Instance.UploadGameData());
        }
    }
}
